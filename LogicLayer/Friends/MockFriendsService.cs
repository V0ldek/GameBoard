...
namespace GameBoard.LogicLayer.Friends
{
    internal sealed class MockFriendsService : IFriendsService
    {
        public Task<IEnumerable<UserDto>> GetFriendsByUserIdAsync(string userId) => userId == "V0ldek" ? Task.FromResult(
            ...
            } as IEnumerable<UserDto>) : Task.FromResult((IEnumerable<UserDto>)null);

        public Task SendFriendRequestAsync(CreateFriendRequestDto friendRequest) => throw new NotImplementedException();

        public Task<FriendRequestDto> GetFriendRequestAsync(string friendRequestId) => throw new NotImplementedException();
