    ...
    internal /*sealed?*/ class FriendService : IFriendsService
    {
        ...
        public Task<IEnumerable<UserDto>> GetFriendsByUserNameAsync(string userName)
        {
            ...
            var smallerIdThanfriends = user
                .Include(u => u.SmallerIdInFriendships)
                .SelectMany(u => u.SmallerIdInFriendships)
                .Where(f => f.FriendshipStatus == FriendshipStatus.Lasts)
                .Include(f => f.UserGreater)
                .Select(f => new UserDto(f.UserGreater.Id, f.UserGreater.UserName, f.UserGreater.Email));

            var greaterIdThanFriends = user
                .Include(u => u.GreaterIdInFriendships)
                .SelectMany(u => u.GreaterIdInFriendships)
                .Where(f => f.FriendshipStatus == FriendshipStatus.Lasts)
                .Include(f => f.UserSmaller)
                .Select(f => new UserDto(f.UserSmaller.Id, f.UserSmaller.UserName, f.UserSmaller.Email));

            var allFriends = smallerIdThanfriends.Union(greaterIdThanFriends).OrderBy(u => u.UserName).ToListAsync();
